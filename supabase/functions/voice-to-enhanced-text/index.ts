...
serve(async (req) => {
  ...
  try {
    ...
    if (!textType) {
      throw new Error('Text type is required (currentSituation, risks, or decisions)');
    }
...
דוגמה: "זוהו מספר פערים חשובים: 1. חוסר כיסוי לאובדן כושר עבודה - עלול לגרום לקושי כלכלי משמעותי. 2. סכום ביטוח החיים נמוך יחסית להכנסה - לא יספק הגנה מספקת למשפחה."`;
    } else if (textType === 'decisions') {
      systemPrompt = `אתה סוכן ביטוח מקצועי. המטרה שלך היא לקחת טקסט גולמי על החלטות שהתקבלו בפגישה ולעצב אותו בצורה מקצועית.

הנחיות:
- כתב בעברית בלבד
- התמקד בפעולות קונקרטיות שהוחלטו
- ציין תאריכים, סכומים ופרטים ספציפיים
- השתמש במשפטים קצרים וברורים
- הוסף מבנה עם נקודות או מספור
- התמקד בצעדים הבאים שיש לבצע
- כתב בצורה מקצועית אבל מובנת

דוגמה: "הוחלט לבצע: 1. הגשת בקשה לביטוח חיים בסך 800,000 ש"ח עד סוף החודש. 2. שדרוג ביטוח הבריאות הקיים לכיסוי מורחב. 3. פגישת מעקב תוזמן לעוד 3 שבועות לחתימה על המסמכים."`;
    }
