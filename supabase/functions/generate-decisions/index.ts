...
2. **רקע ועיקרי הפגישה** - סכם את הרקע הכללי של הפגישה והנושאים המרכזיים שנדונו:
   - תאר את מטרות הפגישה ומה הוסכם במעמד השיחה בפניה ישירה
   - הסבר את הנושאים והשינויים שנדונו, כולל אלו שלא קשורים ישירות למוצרים ספציפיים
   - ציין את הרציונל והשיקולים שהובילו להחלטות בפניה ישירה (למשל: "טווח ההשקעה הפנסיוני שלך הוא ארוך טווח")
   - תאר את הגישה הכוללת והמדיניות שאומצה (למשל: "מטרת השינוי היא הגדלת פוטנציאל התשואה לאורך השנים")
   - 6-8 משפטים מפורטים בפניה ישירה (את/אתה)

3. **מה הוחלט לבצע** - פרט את ההחלטות הקונקרטיות שהתקבלו והסבר את הרציונל בפניה ישירה:
...
חזור עם JSON בפורמט הבא (בלבד, ללא טקסט נוסף):
{
  ...
  "meetingContext": "סיכום כללי של הפגישה והרקע", 
  "decisions": "ניתוח מפורט של ההחלטות הקונקרטיות והרציונל"
}`;
    } else {
...
התשובה צריכה להיות HTML תקין בעברית ומפורטת.`;
    ...
    if (autoFillMode) {
      ...
      try {
        ...
        if (!parsedContent.currentSituation && !parsedContent.meetingContext && !parsedContent.decisions) {
        ...
        return new Response(JSON.stringify({ 
          ...
          meetingContext: parsedContent.meetingContext || '',
