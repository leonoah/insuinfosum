...
export class XMLPensionParser {
  ...
  static async parseXMLFile(file: File): Promise<{
    ...
    const clientName =
      ...
      this.getFirstTextByTags(clientElement || xmlDoc, ["SHEM-MALE", "SHEM-MALE-LAKOACH"]) ||
      "";
    const clientId = this.getFirstTextByTags(clientElement || xmlDoc, [
  ...
  private static parseMutzar(mutzar: Element, index: number): PensionProduct | null {
    ...
    // סוג מוצר
    const sugMutzar =
      this.getFirstTextByTags(mutzar, ["SUG-MUTZAR", "SUG-MUTZAR-TEXT", "SHEM-SUG-MUTZAR", "SUG-MUTZAR-MILULI"]) || "";
    ...
    // חברה
    const company =
      this.getFirstTextByTags(heshbon, ["SHEM-TOCHNIT", "SHEM-GUF-MENAHAL"]) ||
    ...
    if (currentBalance === 0) {
      const yitra = parseFloat(
        this.getFirstTextByTags(heshbon, ["YITRA-NOCHECHIT", "YITRA-NOCHACHIT", "YITRA"]) || "0",
      );
    ...
    // תשואה שנתית
    const annualReturn = parseFloat(this.getElementText(heshbon, "SHEUR-TSUA-NETO", "Tsua") || "0");
    ...
    if ((!employeeDeposit && !employerDeposit) || (isNaN(employeeDeposit) && isNaN(employerDeposit))) {
      const totalLastDeposit = parseFloat(
        this.getElementText(heshbon, "TOTAL-HAFKADA", "PirteiHafkadaAchrona") ||
          this.getElementText(heshbon, "TOTAL-HAFKADA", "PerutPirteiHafkadaAchrona") ||
          "0",
      );
    ...
    // כיסוי ביטוחי
    let deathBenefit = parseFloat(this.getElementText(heshbon, "SCHUM-BITUACH", "PerutMitryot") || "0");
    if (!deathBenefit || isNaN(deathBenefit)) {
      const altDeath = parseFloat(this.getElementText(heshbon, "SCHUM-BITUAH-ZAKAI", "PerutAchreiutKlalit") || "0");
    ...
    // תאריך פתיחת תוכנית
    const planOpenDate = this.formatDate(this.getElementText(heshbon, "TAARICH-HITZTARFUT-MUTZAR"));
  ...
  private static parseHeshbon(mutzar: Element, heshbon: Element, index: number): PensionProduct | null {
    ...
    // חברה
    const company =
      this.getElementText(heshbon, "SHEM-TOCHNIT") ||
    ...
    if (currentBalance === 0) {
      const totalTzviraNeto = parseFloat(this.getElementText(heshbon, "TOTAL-TZVIRA-NETO", "Tzvirot") || "0");
  ...
  private static getElementText(parent: Element | Document | null, tagName: string, subParent?: string): string {
    if (!parent) return "";
  ...
  private static getFirstTextByTags(parent: Element | Document | null, tags: string[], subParent?: string): string {
    for (const t of tags) {
  ...
  private static getProductContainers(xmlDoc: Document): Element[] {
    ...
    xmlDoc
      .querySelectorAll("Mutzar, Mutzarim > Mutzar, PerutMutzarim > Mutzar, PrateiMutzarim > Mutzar")
      .forEach((e) => set.add(e));
    // מכולות שיש בהן חשבון/פוליסה – נעלה לרמה של ההורה
    xmlDoc.querySelectorAll("HeshbonOPolisa, Heshbon-O-Polisa, Heshbon, Polisa").forEach((h) => {
  ...
  private static getFirstByLocalName(parent: Element | Document | null, localName: string): Element | null {
    ...
    const list = (parent as Document | Element).getElementsByTagNameNS?.("*", localName) as
  ...
  private static getAllByLocalName(parent: Element | Document | null, localName: string): Element[] {
    if (!parent) return [];
    const nodeList = (parent as Document | Element).getElementsByTagNameNS?.("*", localName) as
      | HTMLCollectionOf<Element>
      | undefined;
