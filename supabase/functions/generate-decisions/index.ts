...
serve(async (req) => {
  ...
  try {
    ...
    if (autoFillMode) {
      ...
      prompt = `אתה סוכן ביטוח וייעוץ פנסיוני מומחה ומנוסה. תפקידך לנתח מצב ביטוחי ופנסיוני של לקוח ולהסביר לו את ההמלצות בצורה מקצועית, מעמיקה וברורה.
...
אנא מלא את השדות הבאים בטקסט מקצועי ומפורט:

1. **מצב קיים** - נתח את המצב הביטוחי והפנסיוני הנוכחי של הלקוח:
   - תאר את המוצרים הקיימים ומאפייניהם (סוגי מוצרים, דמי ניהול, מסלולי השקעה)
   - הסבר את המשמעות של כל מוצר (למשל, הבדל בין קרן פנסיה לביטוח מנהלים - קרן פנסיה מעניקה פנסיה חודשית בגיל פרישה, ביטוח מנהלים משלב חיסכון עם כיסויים ביטוחיים)
   - ציין את היתרונות והחסרונות של המצב הנוכחי
   - 4-6 משפטים מפורטים

2. **פערים וסיכונים** - זהה ונתח את הפערים והסיכונים:
   - פרט את הפערים הספציפיים במוצרים הקיימים (דמי ניהול גבוהים, כיסויים חסרים, פיזור לא מספק)
   - הסבר את המשמעות של כל פער (למשל, דמי ניהול של 2% על הפיקדון במקום 0.5% פירושו הפסד של אלפי שקלים לאורך השנים)
   - ציין סיכונים ספציפיים ללקוח על בסיס גילו, מצבו המשפחתי והכנסתו
   - 5-7 נקודות מפורטות

3. **מה הוחלט לבצע** - פרט את ההחלטות שהתקבלו והסבר את הרציונל:
   - תאר בדיוק איזה מעבר או שינוי יבוצע (למשל, העברה מקרן פנסיה X לביטוח מנהלים Y)
   - הסבר למה ההחלטה הזו נכונה עבור הלקוח (למשל, ביטוח מנהלים מתאים יותר כי הוא עובד שכיר והוא זקוק לכיסוי ביטוחי נרחב)
   - פרט את היתרונות הצפויים (חיסכון בדמי ניהול, שיפור כיסויים, פיזור סיכונים)
   - ציין לוחות זמנים ופעולות נדרשות
   - 6-8 משפטים מפורטים עם ניתוח מעמיק

**הנחיות קריטיות:**
- אל תמציא מידע שאינך יודע בוודאות (כמו תשואות, ביצועים היסטוריים של קרנות)
- השתמש רק במידע שמופיע בנתוני המוצרים שסופקו
- הסבר כל מונח מקצועי בצורה פשוטה וברורה
- חשוב כסוכן שמסביר ללקוח, לא כרובוט
